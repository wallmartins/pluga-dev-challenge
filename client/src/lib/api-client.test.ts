...
describe('api-client', () => {
  ...
  describe('summariesApi.create', () => {
    it('posts to /summaries and returns data', async () => {
      const mockResponse: any = { id: '123', status: 'pending', summary: null, created_at: '2024-01-01', original_post: 'test' }
      const mockApiClient = { post: jest.fn().mockResolvedValue({ data: mockResponse }) }
      const api = createSummariesApi(mockApiClient as any)

      const result = await api.create({ summary: { original_post: 'test' } })
      expect(result).toEqual(mockResponse)
      expect(mockApiClient.post).toHaveBeenCalledWith('/summaries', { summary: { original_post: 'test' } })
    })
  })

  describe('summariesApi.getById', () => {
    it('gets from /summaries/{id} and returns data', async () => {
      const mockResponse: any = { id: '123', status: 'completed', summary: 'Summary', created_at: '2024-01-01', original_post: 'test' }
      const mockApiClient = { get: jest.fn().mockResolvedValue({ data: mockResponse }) }
      const api = createSummariesApi(mockApiClient as any)
  ...
  describe('summariesApi.list', () => {
    it('gets from /summaries and returns array', async () => {
      const mockResponse: any = [{ id: '1', status: 'completed', summary: 'Summary 1', created_at: '2024-01-01', original_post: 'test' }]
      const mockApiClient = { get: jest.fn().mockResolvedValue({ data: mockResponse }) }
      const api = createSummariesApi(mockApiClient as any)
  ...
  describe('summariesApi (default export)', () => {
    it('is created with default api client', () => {
      // This test exercises the summariesApi which is created with the default api parameter
      // This ensures line 14 (const api = createApiClient()) and line 33 (export const summariesApi = createSummariesApi())
      // are covered in the code coverage report
